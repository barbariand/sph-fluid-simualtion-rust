...
pub fn spiky_smoothing_kernel(distance: f32, smoothing_radius: f32) -> f32 {
    ...
    let volume = PI * smoothing_radius.powi(6);
    (smoothing_radius - distance).powi(3) * 15.0 / volume
}
pub fn spiky_smoothing_kernel_zero_dist(smoothing_radius: f32) -> f32 {
    if smoothing_radius <= 0.0 {
        return 0.0;
    }
    15.0 / (PI * smoothing_radius.powi(3))
}
